...
def read_tasks_from_arc_directory(challenge_dir: str, solution_dir: str = None, max_tasks: int = None) -> List[Task]:
    """
    Read ARC tasks from public directory structure (multiple JSON files)
    challenge_dir: path to directory with challenge JSON files
    solution_dir: path to directory with solution JSON files (optional)
    max_tasks: maximum number of tasks to load (optional)
    """
    challenge_files = sorted(glob.glob(os.path.join(challenge_dir, '*.json')))
    if solution_dir:
        solution_files = sorted(glob.glob(os.path.join(solution_dir, '*.json')))
        solution_map = {os.path.basename(f): f for f in solution_files}
    tasks = []
    for i, challenge_path in enumerate(challenge_files):
        if max_tasks and i >= max_tasks:
            break
        with open(challenge_path, 'r', encoding='utf-8') as f:
            challenge_data = json.load(f)
        # Try to find matching solution file
        solution_data = None
        if solution_dir:
            fname = os.path.basename(challenge_path)
            if fname in solution_map:
                with open(solution_map[fname], 'r', encoding='utf-8') as sf:
                    solution_data = json.load(sf)
        # ARC format: {"train": [...], "test": [...]}
        # Each train/test is a dict with "input" and "output"
        train_examples = [Example.deserialize(ex) for ex in challenge_data.get('train', [])]
        test_examples = [Example.deserialize(ex, test=True) for ex in challenge_data.get('test', [])]
        # If solution_data is present, update test outputs
        if solution_data:
            for idx, sol in enumerate(solution_data.get('test', [])):
                if idx < len(test_examples):
                    test_examples[idx].output = np.array(sol['output'])
        # For ARC, usually one test example per task
        test_example = test_examples[0] if test_examples else None
        task = Task(test_example=test_example, train_examples=train_examples, name=os.path.basename(challenge_path))
        tasks.append(task)
    return tasks
    def __repr__(self) -> str:
